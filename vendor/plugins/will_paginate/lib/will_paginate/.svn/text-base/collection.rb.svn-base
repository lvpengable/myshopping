...
#    end
    def self.create(page, per_page, total = nil)
      pager = new(page, per_page, total)
      yield pager
      if pager.out_of_bounds? #插件自身的方法
        pager = new(pager.total_pages, pager.per_page, total)
        yield pager
      end
      pager
    end
    ...
    # and redirecting.
    def out_of_bounds?
      current_page.to_i > total_pages.to_i
    end
    ...
    #add by wt 显示序号会根据分页增加
    def current_index(obj)
      (current_page-1)*per_page+self.index(obj)+1
    end
  end
end
