  ...
  module Finder
    ...
    protected
      ...
      def wp_count(options, args, finder)
        ...
        # Start add wll  20100604
        if(count_options[:select] == 'distinct contents.id, products.id as product_id')
          count_options[:select] = "distinct contents.id, products.id"
        end
        #End add wll 20100604
        # start change by xzh 20100719
        # counter = Proc.new { count(count_options) }
        counter = Proc.new { count(1, count_options) }
        # end change by xhz
        count = if finder.index('find_') == 0 and klass.respond_to?(scoper = finder.sub('find', 'with'))
                ...
                #start add by tmy 20111018
                elsif finder =~ /_with_deleted/
                  count_with_deleted(1, count_options)
                #end add by tmy 20111018
                else
                  counter.call
                end
